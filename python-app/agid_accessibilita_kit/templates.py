...
def _strip_trailing_slashes(path: str) -> str:
    p = (path or "/accessibilita").strip()
    while p.endswith("/") and len(p) > 1:
        p = p[:-1]
    return p


def render_footer_snippet(base_path: str = "/accessibilita") -> str:
    b = _strip_trailing_slashes(base_path)
    return (
        "<!-- Snippet footer (bozza) -->\n"
        f'<a href="{b}/dichiarazione-accessibilita.html">Dichiarazione di accessibilita</a>\n'
        '<span aria-hidden="true"> | </span>\n'
        f'<a href="{b}/feedback-accessibilita.html">Feedback accessibilita</a>\n'
    )


def render_dichiarazione_html(
    ente: str = "Nome organizzazione",
    sito: str = "https://www.esempio.it",
    email: str = "accessibilita@esempio.it",
    base_path: str = "/accessibilita",
) -> str:
    b = _strip_trailing_slashes(base_path)
